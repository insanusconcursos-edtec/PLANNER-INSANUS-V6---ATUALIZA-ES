...
export const UserDashboard: React.FC<Props> = ({ user, onUpdateUser, onReturnToAdmin }) => {
  ...
  const todayItems = groupScheduleItems(schedule[todayStr] || []);
  ...
  const hasMorePending = todayItems.some(item => !item.completed && item.uniqueId !== gid && item.goalId !== gid);
  ...
  const renderDailyView = () => {
      ...
      const allCompleted = todayItems.length > 0 && todayItems.every(item => item.completed);
